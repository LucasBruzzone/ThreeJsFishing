...
export const HOOK_SPLASH_POINT = new Vector3(0, -1.16, -18)
...
export const HOOK_SINK_END_POINT = new Vector3(0, -42, -22)
...
// Module-scope scratch vectors reused by flightArc to avoid allocating a new
// Vector3 every frame at 60fps.
const apexScratch = new Vector3()
const lerpedStart = new Vector3()
const lerpedEnd = new Vector3()
...
export const flightArc = (
  ...
  apexScratch.lerpVectors(rodTipWorld, HOOK_SPLASH_POINT, 0.5)
  apexScratch.y += FLIGHT_APEX_HEIGHT
  lerpedStart.lerpVectors(rodTipWorld, apexScratch, flightProgress)
  lerpedEnd.lerpVectors(apexScratch, HOOK_SPLASH_POINT, flightProgress)
  output.lerpVectors(lerpedStart, lerpedEnd, flightProgress)
}

// Eased descent: fast just after splash (entry momentum), then steady.
export const descentEase = (progress: number) => 1 - Math.pow(1 - progress, 1.6)
